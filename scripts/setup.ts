...
async function setup() {
  ...
  if (envFileExists) {
    const overwrite = await askQuestion(
      "❓ .env.local already exists. Do you want to update it? (y/n): ",
    );
  ...
  if (createEnvFile) {
    ...
    const nextAuthUrl = await askQuestion(
      `❓ Enter your NEXTAUTH_URL (default: ${defaultUrl}): `,
    );

    // Generate NEXTAUTH_SECRET
    const nextAuthSecret = generateSecureString();
    console.log(
      `✅ Generated NEXTAUTH_SECRET: ${nextAuthSecret.substring(0, 8)}...`,
    );

    // Get DATABASE_URL
    const defaultDbUrl = "postgres://user:password@hostname:port/database";
    const dbUrl = await askQuestion(
      `❓ Enter your DATABASE_URL (example: ${defaultDbUrl}): `,
    );
